...
// diagram lightbox: click to enlarge, X / backdrop / Esc to close
const lb=document.createElement('div');lb.className='lightbox';
lb.innerHTML='<button class="x" aria-label="Close">&times;</button><img alt="">';
b.appendChild(lb);const lbImg=lb.querySelector('img');
function closeLb(){lb.classList.remove('open');b.classList.remove('lb-open');}
lb.addEventListener('click',e=>{if(e.target!==lbImg)closeLb();});
document.addEventListener('keydown',e=>{if(e.key==='Escape')closeLb();});
document.querySelectorAll('.content img').forEach(im=>im.addEventListener('click',()=>{
  lbImg.src=im.currentSrc||im.src;lb.classList.add('open');b.classList.add('lb-open');}));
</script>"""
